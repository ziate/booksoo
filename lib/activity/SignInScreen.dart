    ...
    await isNetworkAvailable().then((bool) async {
      if (bool) {
        ...
        await getLoginUserRestApi(request).then((res) {
          ...
          setValue(USER_EMAIL, response.userEmail!);
          setValue(USERNAME, response.userNicename!);
          setValue(TOKEN, response.token!);
          setValue(AVATAR, response.avatar!);
          if (response.profileImage != null) {
            setValue(PROFILE_IMAGE, response.profileImage!);
          }
          ...
          setValue(USER_DISPLAY_NAME, response.userDisplayName!);
  ...
  void socialLogin(req) async {
    ...
    await socialLoginApi(req).then((response) async {
      ...
      await getCustomer(response['user_id']).then((res) {
        ...
        setValue(USER_EMAIL, response['user_email']);
        setValue(USERNAME, response['user_nicename']);
        setValue(TOKEN, response['token']);
        setValue(USER_DISPLAY_NAME, response['user_display_name']);
        setValue(IS_LOGGED_IN, true);
        setState(() {
          isLoading = false;
        });
